...
{\parskip=0.2cm\par\unskip\centering}
{\parskip=0pt\par\unskip\noindent}
...
\begin{titlepage}
	\begin{center}
		...
		\noindent
		{\large  A Thesis Conducted to Acquire a Bachelor Degree In Computer Science 
...
\begin{blockfigure}{Graph Diagram for an Example Dummy ConvNet}
	\begin{center}
...
\end{lemma}
\begin{algorithm}
	\caption[]{1D Convolution Pseudo-Code\protect\footnotemark}
	\begin{algorithmic}
		\State $ step = 0 $
	...
	\end{algorithmic}
\end{algorithm}
\footnotetext{This Pseudo-Code was reconstructed from examples designed to understand 1D Convolution, implemented by the author in Tensorflow and Python. The source code for the Tensorflow conv1d operation was studied briefly, and it seems it is implemented by forcing conv2d to perform convolution over just one axis in the batch.}
\newpage
...
From definition \ref{definition:\thedefinition}, we consider ~$ D = \{X , P (X)\} $, the data sets (that is $ X $) (key batch, message (plain) batch) and (key-seed batch, message (plain) batch) are the same, in the sense that they have the same sizes and they are both randomly generated, they also both follow the same normalized binary format (-1, 1).\\
...
Also from definition \ref{definition:\thedefinition}, we consider ~$ T = \{Y, P (Y |X)\} $, the label spaces $ Y_{S} $, $ Y_{T} $ are equal since the source label space (asymmetric) is the same as the target label space (symmetric), in the sense that they both deal with (plain, key) labels as input and (predicted plain) labels as output.\\
...
\end{conjecture}
\paragraph{\textbf{Testing}}\mbox{}\\
In the testing setting, all neural nets are reconstructed with the same internal structure, but they differ in how they handle input and output, to be geared towards symmetric cryptography.\\
