				...
				enhanced,size=small,fit algorithm=hybrid* ]
					...
					\begin{tcolorbox}[
						...
						title = {Supervisor}
							...
							\large
							\emph{Name :} \\[0.2cm]
							\tab Prof.~Giovanni \textsc{De Gasperis}\\[0.2cm]
					\end{tcolorbox}
			\end{tcbraster}
			\vfill
		\today
...
\begin{titlepage}	
	...
	\vspace{-0.45cm}
	\begin{nscenter}
		The author of this dissertation retains its copyrights (of both source and compiled forms) and releases it under a\\
	...
	\vspace{-0.45cm}
	\begin{nscenter}
		The project presented in this dissertation, as well as its code (in both source and compiled forms) is under the following copyrights terms:
	\end{nscenter}
	\vspace{-0.5cm}
	\begin{tcbraster}[raster columns=2,raster rows=1,
		enhanced,size=small,fit algorithm=hybrid* ]
	...
	\vspace{-0.6cm}
		\begin{itemize}[nosep]
		\item A copy of this dissertation will \textit{initially} be published on the author's Github account.
...
\begin{definition}
	(Neural Cryptography): is an interdisciplinary field in Computer Science, combining both artificial intelligence and cryptology, it is focused on the development of stochastic methods, based on artificial neural networks, for use in cryptography and cryptanalysis.
\end{definition}
...
\end{itemize}
With the research spectrum in this area being dominated by authors coming from a mathematical-background, my dissertation aims to provide:
\begin{itemize}
...
The work being done so far in neural cryptography can be divided to old (pre 2016), and new (post 2016). For the old section, this dissertation will only list the works and attributions without delving into the details.
...
This chapter deals with the mechanisms behind convolutional neural nets, which are the building blocks for the adversarial crypto-system built for this thesis.\\
This chapter serves to provide a theoretical base for much of the basics and the enhancements researched for this thesis.\\
...
\end{lemma}
\newpage
Suppose we have an input X from a previous layer with $ n_{in} $ components and a neuron $ n $ with random weights W in the current layer which spits out the same output Y to $ n_{out} $ neurons in the next layer.
...
\begin{center}
	\begin{equation}
	 Var(Y) = Var(X_i), n = n_{in} \implies Var(W_i) = \frac{1}{n_{in}} \implies Var(W_i) * n_{in} = 1  \label{eq:5}
	\end{equation}
\end{center}
...
Testing the Pseudo-Code by implementing it from scratch will be the focus of further research, but for the purpose of this thesis it was sufficient to see how it works as a result of a higher-level Tensorflow based implementation, because it is not the focus of the thesis, but rather a side project to determine the efficacy of convolution in neural cryptography.\\
The source code for the Tensorflow 1D Convolution operation (conv1d) was studied briefly, and it seems it is implemented by forcing 2D Convolution (conv2d) to perform convolution over just one axis in the batch, but in this thesis the focus will be on reconstructing 1D Convolution from scratch.
\section{\textbf{1D Convolution on Batched 1D Data with 2D Filters}}
...
In order for the convolution to do these multiplications, it \textbf{slides} over one axis of the 1D data sample, the y-axis, and the filter also \textbf{slides} itself to fit the location.\\
...
It is also important to note that since the stride determines how many steps to slide across for each new location, it is synonymous to dividing the length of the y-axis of the final input by the stride length, then readjusting the y-axis to its original length by consuming the z-axis length.\\
...
\begin{itemize}[nosep]
	...
	\item $ result.yAxis.length = batch.yAxis.length $
	\item $ result.zAxis.length = filter.zAxis.length $
\end{itemize}
Each entry over the x-axis of the result represents a processed sample, and if the z-axis of the result has $ length > 1 $, then each entry over the y-axis represents a feature map of the sample, each feature map has different derived information from the spatial location that was processed.\\
...
\begin{lemma}\label{lemma5}
	if $ filter.xAxis.length > batch.yAxis.length $, the filter is offset by an amount of: $ filter.offset =  filter.xAxis.length - batch.yAxis.length $, throughout the convolution process.
\end{lemma}
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\EndIf
		\State $ x = 0 $
		\While {$ x < batch.xAxis.length $}
			\State $ y = 0 $
			\While {$ y < batch.yAxis.length $}
				\State $ z = 0 $
				\While {$ z < filter.zAxis.length $}
					\begin{flalign*}
						b_{end} &= batch.yAxis.length\\
						f_{start} &= (filter.offset - y >= 0)~ ?~ (filter.offset - y) : 0\\
						f_{end} &= f_{start} + b_{end}\\
						b_{start} &= (f_{start} > 0)~ ?~ 0 : y \\ 
						\tab result[x][y][z] &=
						\sum_{i=f_{start}}^{i<f_{end}}~
						\sum_{j=b_{start}}^{j<b_{end}}
						\biggl\{
						batch[x][i-f_{start}][j] * filter[i][j][z]
						\biggr\}\protect\footnotemark
					\end{flalign*}
					\State $ z = z + 1 $
				\EndWhile
				\State $ y = y + stride $
	...
	\end{algorithmic}
\end{algorithm}
\footnotetext{\scriptsize{An out of bound error may arise from this equation, which can be easily avoided when the equation is expanded to loops.}}
\newpage
...
It also illustrates how 1D Convolution works on batched 1D data with 2D (expanded to 3D) filters in each Conv Layer.\\\\
...
\end{tcbraster}
\newpage
After the input batch tensor has been provided and the filter tensor has been generated, the convolution process can begin.\\
...
\begin{itemize}[nosep]
	...
	\item Each function is fed the result of matrix multiplication operations (involving floating-point weight values), therefor each function receives floating-point values as input.
	\item Sigmoid and LeakyRelu results are then fed to next layers, therefor their results are multiplied by a distribution of floating-point values.
...
\begin{hypothesis}\label{hypothesis:1}
	the net can be considered as an information channel, and Shannon's theory \citep{Shannon:2001:MTC:584091.584093} applies to it, therefor the net can be analyzed by its entropy.
\end{hypothesis}
\begin{numericalanalysis}\label{analysis:1}
	considering a floating-point distribution over an input x-axis, the distribution of possible result values over the y-axis can be described as the following:
...
\begin{suggestedproof}
	Shannon's entropy equation states that:
	\begin{center}
		$ H(V) = - \Sigma \space P(v_{i})*log_{b}(P(v_{i})) $
	\end{center}
	And since line-spaces can be converted to discrete values, Shannon's entropy equation can be applied to the line-spaces in Numerical-Analysis~\ref{analysis:1}, as the following:\\
	\begin{center}
		$ \forall x \in X, X = [-1, +1]: H(Y) = - \Sigma \space P(y_{i})*log_{b}(P(y_{i})) $
	\end{center}
	\begin{itemize}
		\item $" Sigmoid \rightarrow LeakyRealu \rightarrow Sigmoid "$ is equiprobable as we move along the x-axis, but it has no information capacity for negative values and it has a smaller $ H(Y) $ value.
		\item $" Tanh \rightarrow LeakyRealu \rightarrow Tanh "$ begins to become non-linearly inequiprobable as we move towards 0 along the x-axis, this not only results in less information capacity and a lower $ H(Y) $, but because each neuron in a layer is independent of the other, therefor each value probability along the x-axis is independent of the other, and the net could be training on a mix of values randomly chosen along the x-axis, this will lead to non-uniformity of information capacity and entropy across the trainable process (e.g. Conv1D), which will lead to inconsistent results for that process, and the convergence will tick high and low, therefor probably not get fully achieved.
		\item $" Sigmoid \rightarrow LeakyRealu \rightarrow Tanh "$ is equiprobable as we move along the x-axis, it has complete information capacity for the range of negative values which the nets deal with, and it has a larger $ H(Y) $ value. 
	\end{itemize}
\end{suggestedproof}
\newpage
...
\begin{itemize}
	...
	\item Decryptors: Bob.
	\item Eavesdroppers: Eve (known helpful adversary), Alan (unknown malicious adversary).
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Input Layer:} a series of sequential fully connected layers to teach the net about cryptanalysis, which is 1 layer in case of: Alice, Bob, Key Generators, but for Eve and Alan it is: 
	\begin{center}
		$ \sum_{i = 0}^{i=n} No._{fcl}[i] $ ~ Where ~
		$ n \in [Alice, Bob, Key ~ Generators] $
	\end{center}
	That is to give both eavesdroppers an edge in cryptanalysis.\\
	Each input layer has a weight shape of $ (16 ~ or ~ 32, 32) $, which makes it normalize the shape of the data passed to the convolutional layer to become $ (512, 32) $.
	\item \textbf{Dimensionality Expansion:}  the shape of the data passed to the convolutional layer to become $ (512, 32, 1 $).
	\item \textbf{Convolutional Layers:} 4 layers, among the first three there is just one with stride of value 2 to reduce the shape of the data to $ (512, 16, 1) $ and they have leakyRelu as activation functions, the final layer has the hyperbolic tangent as an activation function.
...
\end{blockfigure}
\newpage
\begin{blockfigure}{ Net Graph Diagram - Key Generators}
	\begin{center}
...
\end{itemize}
In the training setting, the Cipher is passed directly from the Encryptor to Bob, Eve and Alan, without being intermediated in between, because back-propagation does not work past static blocks of data (which is the case for intermediately passed Ciphers).\\
...
\begin{itemize}
	...
	\item Alice is fed the pubKey and the plaintext message and produces a Cipher.
...
\end{itemize}
In the training setting, the Cipher is passed directly from the Encryptor to Bob, Eve and Alan, without being intermediated in between, because back-propagation does not work past static blocks of data (which is the case for intermediate Ciphers).
The same goes for the privKey-pubKey pair: 
\begin{itemize}
	\item The pubKey is passed directly to the privKey Generator, Alice, Eve and Alan without being intermediated.
	\item The privKey is passed directly from the privKey Generator to Bob without being intermediated.
\end{itemize}
...
\begin{itemize}[nosep]
	\item $ \textbf{X} $\textbf{:} the data sets (key batch, plaintext batch) and (key-seed batch, plaintext batch) are the same between schemes, in the sense that they have the same sizes and they are randomly generated, they also follow the same normalized binary format (-1, 1).
...
\begin{itemize}[nosep]
	\item $ \boldsymbol{Y_{S}} $\textbf{,} $ \boldsymbol{Y_{T}} $\textbf{:} are equal since the source label space (asymmetric) is the same as the target label space (symmetric), in the sense that they both deal with (plaintext, key) labels as input and (predicted-plaintext) labels as output.
...
The schemes presented in this thesis and the experiments they produce, were implemented with an object oriented design: methods that perform the exact same thing for all schemes were packed into one base class, the parameter loading was handled with one class, and the rest was implemented with different classes.\\\\
...
\begin{colortable}{Underlying Hardware Infrastructure}
	\textbf{Device} & Graphics Card \\\hline
	\textbf{Manufacturer} & Nvidia \\\hline
	\textbf{Name \& Number} & GeForce 920M \\\hline
...
\begin{itemize}[nosep]
	...
	\item The role of modern modes of cryptographic operations, which are considered an important corner stone in modern cryptography, further research into teaching neural nets how to synthesize rigorous modes of operation needs to be done, as a starting point, we can consider studying the CTR Mode of block-ciphers.~\citep{BlockCipherModes}
...
\begin{center}
	\begin{minipage}{\textwidth}
		I would like to thank the following entities for their direct or indirect help on this thesis:
		\begin{itemize}[label={}]
			\item \textbf{Prof. Giovanni De Gasperis}, Senior Researcher \& Assistant Professor in the Department of Information Engineering, Computer Science and Mathematics at University of L'Aquila: for his guidance, patience, deep academic engagement and his supervision of this thesis.
			...
			\item \textbf{Alexander Lenail}, B.S. Computer Science from Tufts University: for his correspondence to my email regarding his work on illustrating neural network architectures.
		\end{itemize}
	\end{minipage}
\end{center}
\begin{center}
	\begin{minipage}{\textwidth}
		...
		\begin{itemize}[label={}]
			\item \textbf{Those who helped me academically:} professors, lecturers, lab technicians, and general academic staff.
			\item \textbf{Those who helped me professionally:} trainers, tutors and supervisors.
			\item \textbf{Those who helped me financially:} scholarship organizers \& providers.
			\item \textbf{Those who provided moral support:} including friends \& family.
		\end{itemize}
	\end{minipage}
\end{center}
\vfill
